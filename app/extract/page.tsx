...
import { extractColorsFromImage, type ExtractedColor } from "@/lib/image-color-extractor"
...
export default function ExtractPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      {/* Header */}
      <header className="border-b-4 border-black p-6 bg-white">
            ...
            <div className="space-y-4">
              <div className="border-4 border-black p-8 bg-gray-50">
                <label className="block mb-4">
                  <span className="text-sm font-bold mb-2 block">NUMBER OF COLORS: {colorCount}</span>
                  <input
                    type="range"
                    min="3"
                    max="10"
                    value={colorCount}
                    onChange={(e) => setColorCount(Number.parseInt(e.target.value))}
              ...
              {imagePreview && (
                <div className="border-4 border-black">
                  <img src={imagePreview || "/placeholder.svg"} alt="Uploaded" className="w-full h-auto" />
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
                    ...
                    {extractedColors.map((color, index) => (
                      ...
                      >
                        <div className="flex-1 p-6">
                          <p className="text-2xl font-bold" style={{ color: getContrastColor(color.hex) }}>
                            {color.hex}
                          </p>
                          <p className="text-sm font-bold" style={{ color: getContrastColor(color.hex) }}>
              ...
              {!isProcessing && extractedColors.length === 0 && (
                <div className="border-4 border-black p-12 text-center bg-gray-50">
                  <p className="font-bold text-lg">UPLOAD AN IMAGE TO EXTRACT COLORS</p>
                </div>
              )}
            </div>
          </div>
